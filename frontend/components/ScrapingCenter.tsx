      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          <button
            ...
            className={`px-4 py-2 rounded-xl flex items-center gap-2 transition-all shadow-sm ${isProcessing ? 'bg-slate-200 text-slate-500 cursor-not-allowed' : 'bg-indigo-600 text-white hover:bg-indigo-700'
              }`}
          >
          ...
          <button
            onClick={() => alert("Funcionalidad de Scraper en desarrollo. Pronto estará disponible.")}
            className="bg-emerald-600 text-white px-4 py-2 rounded-xl flex items-center gap-2 hover:bg-emerald-700 transition-all shadow-sm"
          >
            <Play size={18} />
            Lanzar Scraper
          </button>
              ...
              {state.grupos.map(grupo => (
                <div key={grupo.id} className="p-6 flex items-center justify-between hover:bg-slate-50 transition-colors">
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <button
                      onClick={() => alert(`Configuración para ${grupo.nombre} no disponible aún.`)}
              ...
              {state.tareas.map(tarea => (
                <div key={tarea.id} className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <button onClick={() => alert("Configuración de tarea no implementada.")} className="text-slate-400"><Settings2 size={14} /></button>
                  </div>
